...
def chat():
    ...
    mark_node_visited(chat_nodes, current_coords[0], new_coords[1])
    save_chat_nodes(chat_id, chat_nodes)
 ...
 """
    ...
    look_match = re.search(
        r'@LOOK:\s*(northeast|northwest|southeast|southwest|north|south|east|west)',
        user_message,
        re.IGNORECASE
    )
    ...
    if look_match:
        ...
        current_node_data = get_node_by_coords(current_coords[0], current_coords[1], chat_nodes)
        if current_node_data and "connections" in current_node_data and move_key in current_node_data["connections"]:
            look_coords = current_node_data["connections"][move_key]
            look_node = get_node_by_coords(look_coords[0], look_coords[1], chat_nodes)
            if look_node:
                desc = look_node.get("description_base", "")
                first_sentence = desc.split(".")[0] + "." if "." in desc else desc
                look_result = f"You peek {look_direction_str.lower()} and see... {first_sentence}"
            else:
                look_result = f"You peek {look_direction_str.lower()} but see nothing of note."
        else:
            look_result = f"You can't look {look_direction_str.lower()} from here."

        return jsonify({
        'chat_id': chat_id,
        'response': "",
        'narrator_message': "",
        'look_result': highlight_text(look_result),
    ...
    if narrator_message:
        ...
        chat['messages'].append({
            'role': 'system',
            'display_as': 'narrator',           # Add this new field for the frontend
            'content': narrator_full,
            'timestamp': time.time()
        })
        ...
        if char_current_node_data and "connections" in char_current_node_data:
            ...
            if move_key and move_key in connections:
                ...
                if new_node_data:
                    ...
                    new_node_data.setdefault('gameplay', {})['visited'] = True
                    mark_node_visited(chat_nodes, llm_new_coords[0], llm_new_coords[1])
                    save_chat_nodes(chat_id, chat_nodes)
    ...
    response_text = highlight_text(response_text)
    ...
    if final_narrator_message:
        final_narrator_message = highlight_text(final_narrator_message)
    
    if final_narrator_message:
        chat['messages'].append({
            'role': 'system',
            'display_as': 'narrator',           # Add this new field for the frontend
            'content': final_narrator_message,
            'timestamp': time.time()
        })
    ...
    if look_result:
        api_response['look_result'] = look_result
